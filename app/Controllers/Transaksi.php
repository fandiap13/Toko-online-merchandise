...
class Transaksi extends BaseController
{
  ...
  public function pembayaranMidtrans()
  {
    if ($this->request->isAJAX()) {
      ...
      // startwith
      if (!str_starts_with($notelp, '08')) {
        $json = [
          'errors' => [
            'notelp' => 'No. telp harus diawali dengan angka 08',
          ]
        ];
        echo json_encode($json);
        exit();
      ...
      $valid = $this->validate([
        ...
        'notelp' => [
          ...
          'errors' => [
            'required' => '{field} tidak boleh kosong',
            'numeric' => '{field} harus terdiri dari angka',
      ...
      if ($valid) {
        ...
        $json = [
          'errors' => [
            ...
            'ekspedisi' => $validation->getError('ekspedisi'),
            'paket' => $validation->getError('paket'),
  ...
  public function daftar_transaksi()
  {
    ...
    if ($cektransaksi->getNumRows() > 0) {
      ...
      $datasetting = $Modelsetting->select('kontak')->limit(1)->orderBy('id', 'ASC')->get()->getRowArray();
      $kontak = explode("#", $datasetting['kontak']);
      $noWa = "62" . substr($kontak[1], 1);
