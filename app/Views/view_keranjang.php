        ...
        <div class="card">
          ...
          <div class="card-body">
            ...
            <div class="form-group">
              ...
              <div class="invalid-feedback errorekspedisi"></div>
            </div>
            <div class="form-group">
              <label for="">Paket</label>
              <select name="paket" id="paket" class="form-control" disabled></select>
              <div class="invalid-feedback errorpaket"></div>
            </div>
            <div class="form-group">
              ...
              <button type="submit" name="checkout" class="btn btn-block btn-success btnCheckOut" disabled><i class="fa fa-check-circle"></i> Checkout (<b id="totalcheckout"></b>)</button>
              <?= form_close(); ?>
            </div>
          </div>
        </div>
...
<script>
  ...
  function totalcheckout() {
    ...
    // console.log(parseInt(totalbelanja));
    // console.log(parseInt(totalbelanja) > 0);
  ...
  $(document).ready(function() {
    ...
    $('.formcheckout').submit(function(e) {
      ...
      // console.log(alamatpengiriman.trim().length);

      if (
        notelp.trim().length == 0 ||
        alamatpengiriman.trim().length == 0 ||
        provinsi.trim().length == 0 ||
        distrik.trim().length == 0 ||
        ekspedisi.trim().length == 0 ||
        paket.trim().length == 0 ||
        totalberat.trim().length == 0 ||
        provinsiterpilih.trim().length == 0 ||
        distrikterpilih.trim().length == 0 ||
        tipeterpilih.trim().length == 0 ||
        kodeposterpilih.trim().length == 0 ||
        ekspedisiterpilih.trim().length == 0 ||
        paketterpilih.trim().length == 0 ||
        ongkir.trim().length == 0 ||
        estimasi.trim().length == 0 ||
        totalpembelian.trim().length == 0 ||
        totalbayar.trim().length == 0
      ) {
        ...
        $.ajax({
          ...
          success: function(response) {
            ...
            if (response.errors) {
              if (response.errors.notelp) {
                $('#notelp').addClass('is-invalid');
                $('.errornotelp').html(response.errors.notelp);
              }
              if (response.errors.alamat) {
                $('#alamatpengiriman').addClass('is-invalid');
              ...
              if (response.errors.distrik) {
                $('#notelp').addClass('is-invalid');
                $('.errornotelp').html(response.errors.distrik);
              }
              if (response.errors.ekspedisi) {
                $('#ekspedisi').addClass('is-invalid');
                $('.errorekspedisi').html(response.errors.ekspedisi);
              }
